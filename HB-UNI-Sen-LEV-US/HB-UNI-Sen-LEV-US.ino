...
#define SENSOR_EN_PIN    14 //A0
#define SENSOR_RX_PIN    4
#define SENSOR_TX_PIN    3
#define SENSOR_MAX_WAIT 1000              // max time (ms) delay to wait on sensor response 
#define SENSOR_TRIG_PIN SENSOR_TX_PIN     // keep compatibilty with MAXSONAR and HC-SR204 types
#define SENSOR_ECHO_PIN SENSOR_RX_PIN     // keep compatibilty with MAXSONAR and HC-SR204 types
#define BATT_EN_PIN        19             // A5 Pin for battery measure (ground)
#define BATT_SENS_PIN      A7             // Voltage measure
...
NeoSWSerial   sensor(SENSOR_RX_PIN, SENSOR_TX_PIN);
...
const struct DeviceInfo PROGMEM devinfo = {
  {0xF9, 0xD2, 0x11},          // Device ID
  "HMLEV00001",                // Device Serial
    ...
    void measure() {
      ...
      switch (this->getList1().sensorType()) {
        ...
        case US100:
          sensor.listen();
          sensor.flush();
          DPRINTLN(F("US-100: write distance request"));
          sensor.write(0x55);
          DPRINT(F("US-100: Start wait: ")); DDECLN(millis());
          for ( int i = 0; (i < SENSOR_MAX_WAIT) && !sensor.available(); i++ ) delayMicroseconds(1);
          DPRINT(F("US-100: End wait: ")); DDECLN(millis());
          if (sensor.available()) {
            highDist = sensor.read();    // get high byte
            for ( int i = 0; (i < SENSOR_MAX_WAIT) && !sensor.available(); i++ ) delayMicroseconds(1);
            if (sensor.available()) {
              lowDist = sensor.read();   // get low byte
              mmDist = highDist * 256 + lowDist;
            }
            else {
              DPRINT(F("Incomplete response to distance request"));
              mmDist = 0;
            }
          }
          else {
            DPRINT(F("No response to distance request"));
            mmDist = 0;
          };
          m_value = mmDist / 10L;
...
void sensorISR() {
  NeoSWSerial::rxISR( *portInputRegister( digitalPinToPort( SENSOR_RX_PIN ) ) );
}
...
void setup () {
  ...
  DPRINTLN(F("Start interrupt config: "));
  sensor.begin(9600);
  enableInterrupt(SENSOR_RX_PIN, sensorISR, CHANGE);
